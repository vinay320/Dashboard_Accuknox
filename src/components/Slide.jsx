...
//     state.search.isSearching ? state.search.filteredResults : state.widget.categories
//   );

//   const isModalOpen = useSelector((state) => state.modal.isOpen); // Update this to use modal slice

//   const handleOpenModal = () => {
//     dispatch({ type: "modal/open" }); // Use the correct action type from your modal slice
//   };

//   const handleCloseModal = () => {
//     dispatch({ type: "modal/close" }); // Use the correct action type from your modal slice
//   };
...
//                       No data available <img src={graph} width={100} height={100} />
//                     </p> 
//                   )}
//                 </div>
//               ))}
...
const Slide = () => {
  ...
  // Access the combined state correctly
  const { categories, isSearching, filteredResults } = useSelector(
    (state) => state.combine
  );

  const isModalOpen = useSelector((state) => state.modal.isOpen); // Adjust if needed

  const handleOpenModal = () => {
    dispatch({ type: "modal/open" }); // Use correct action type from your modal slice
  };

  const handleCloseModal = () => {
    dispatch({ type: "modal/close" }); // Use correct action type from your modal slice
  };
